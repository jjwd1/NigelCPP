...
namespace NGL {
	...
	inline torch::optim::Optimizer* MakeOptimizer(ModelOptimType type, const std::vector<torch::Tensor>& parameters, float lr) {
		switch (type) {
		case ModelOptimType::ADAM: return new torch::optim::Adam(parameters, lr);
		case ModelOptimType::ADAMW: return new torch::optim::AdamW(parameters, lr);
		case ModelOptimType::ADAGRAD: return new torch::optim::Adagrad(parameters, lr);
		case ModelOptimType::RMSPROP: return new torch::optim::RMSprop(parameters, lr);
		case ModelOptimType::MAGSGD: return new MagSGD(parameters, lr);
		}
	...
	inline void SetOptimizerLR(torch::optim::Optimizer* optimizer, ModelOptimType type, float lr) {
		for (auto& group : optimizer->param_groups()) {
			switch (type) {
			case ModelOptimType::ADAM: static_cast<torch::optim::AdamOptions&>(group.options()).lr(lr); break;
			case ModelOptimType::ADAMW: static_cast<torch::optim::AdamWOptions&>(group.options()).lr(lr); break;
			case ModelOptimType::ADAGRAD: static_cast<torch::optim::AdagradOptions&>(group.options()).lr(lr); break;
			case ModelOptimType::RMSPROP: static_cast<torch::optim::RMSpropOptions&>(group.options()).lr(lr); break;
			case ModelOptimType::MAGSGD: static_cast<MagSGDOptions&>(group.options()).lr(lr); break;
			default: RG_ERR_CLOSE("Unknown optimizer type: " << (int)type);
			}
		}
	}
	...
	public:
		...
		Model() : config(PartialModelConfig{}), device({}), modelName(NULL) {}
		...
		std::filesystem::path GetSuffixedSavePath(std::filesystem::path folder, std::string suffix) const {
			...
			for (char& c : filename) c = toupper(c);
			filename += ".lt";
			return folder / filename;
		}

		std::filesystem::path GetSavePath(std::filesystem::path folder) const { return GetSuffixedSavePath(folder, ""); }
		std::filesystem::path GetOptimSavePath(std::filesystem::path folder) const { return GetSuffixedSavePath(folder, "_optim"); }
		...
		uint64_t GetParamCount() {
			...
			for (auto& param : this->parameters())
				if (param.requires_grad()) total += param.numel();
			return total;
		}
	...
	public:
		...
		Model* operator[](const std::string& name) {
			...
			return (itr == map.end()) ? NULL : map[name];
		}

		void Add(Model* model) { map[model->modelName] = model; }

		void StepOptims() { for (Model* model : *this) model->StepOptim(); }

		void Save(std::filesystem::path folder, bool saveOptims = true) {
			for (Model* model : *this) model->Save(folder, saveOptims);
		}

		void Load(std::filesystem::path folder, bool allowNotExist, bool loadOptims) {
			for (Model* model : *this) model->Load(folder, allowNotExist, loadOptims);
		}
		...
		ModelSet CloneAll() {
			ModelSet clone = *this;
			for (Model*& model : clone) model = model->MakeClone();
			return clone;
		}

		void Free() {
			for (Model* model : *this) delete model;
			map.clear();
		}
	};
}
