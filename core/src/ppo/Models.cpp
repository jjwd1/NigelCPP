...
NGL::Model::Model(
	const char* modelName, ModelConfig config, torch::Device device) :
...
torch::Tensor NGL::Model::Forward(torch::Tensor input, bool halfPrec) {
	...
	if (halfPrec) {
		if (_seqHalfOutdated) {
			...
			if (seqHalf->size() == 0) {
				for (auto& mod : *seq) seqHalf->push_back(mod.clone());
...
void NGL::Model::SetOptimLR(float newLR) {
	SetOptimizerLR(optim, config.optimType, newLR);
}

void NGL::Model::StepOptim() {
	optim->step();
	optim->zero_grad();
	_seqHalfOutdated = true;
}

void NGL::Model::Save(std::filesystem::path folder, bool saveOptim) {
...
void NGL::Model::Load(std::filesystem::path folder, bool allowNotExist, bool loadOptim) {
	std::filesystem::path path = GetSavePath(folder);
	...
	if (!std::equal(sizesBefore.begin(), sizesBefore.end(), sizesAfter.begin(), sizesAfter.end())) {
		...
		for (int i = 0; i < 2; i++) {
			...
			for (uint64_t size : (i ? sizesAfter : sizesBefore)) stream << size << ' ';
			stream << " ]";
			if (i == 0) stream << ",\n";
		}
...
torch::Tensor NGL::Model::CopyParams() const {
