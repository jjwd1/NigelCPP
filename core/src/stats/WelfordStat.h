...
namespace NGL {
	...
	struct BatchedWelfordStat {
		...
		void Reset() { *this = BatchedWelfordStat(width); }

		const std::vector<double>& GetMean() { return runningMeans; }

		std::vector<double> GetSTD() {
			if (count < 2) return std::vector<double>(width, 1);
