...
private:
	...
	std::function<void(std::string)> ArgumentCallback;
...
public:
	...
	QueueData(const std::string& command, const std::string& arguments, bool bInternal, uint64_t delay, uint32_t multiplier = 60);
...
{
private:
	static inline std::map<VariableIds, std::string> VariableMap;
