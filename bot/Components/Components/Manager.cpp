...
void ManagerComponent::UnrealCommand(const std::string& unrealCommand, bool bLogToConsole)
{
	...
	if (defaultActor)
	{
		if (unrealCommand.find("unreal_command") == 0)
		{
			...
			defaultActor->ConsoleCommand(Format::ToFString(formattedCommand), 1);
		}
		else
		{
			if (bLogToConsole)
			...
			defaultActor->ConsoleCommand(Format::ToFString(unrealCommand), 1);
		}
	}
}
...
std::pair<CommandTypes, std::string> ManagerComponent::InternalCommand(const QueueData& queueData, ThreadTypes thread)
{
	...
	{
		CommandQueue.push_back(queueData);
...
void ManagerComponent::OnTick()
{
	if (!CommandQueue.empty())
	{
		...
		for (QueueData& data : CommandQueue)
		{
			if (!data.Expired)
			{
				if (data.IsAsync())
		...
		if (safeToClear && !CommandQueue.empty())
		{
			CommandQueue.clear();
		}
	}
...
void ManagerComponent::PrintModule(const std::string& moduleName)
{
	if (ModuleMap.find(moduleName) != ModuleMap.end())
	{
		...
		Console.Notify("module name: " + mod->GetName());
		Console.Notify("module description: " + mod->GetDescription());
		Console.Notify("allowed states and/or permissions: " + allowedStates);
		Console.Notify("allowed in current state: " + std::string((mod->IsAllowed() ? "true" : "false")));
	}
	else
	{
		Console.Error("cant find module \"" + moduleName + "\"");
	}
}
...
{
	name = Format::ToLower(name);

	for (auto variableIt = VariableMap.begin(); variableIt != VariableMap.end(); variableIt++)
	{
		if (variableIt->second == name)
		{
...
template std::shared_ptr<NigelUI> ManagerComponent::CreateModule<NigelUI>(Module* mod, std::shared_ptr<NigelUI>& moduleToBind);
template std::shared_ptr<NigelUI> ManagerComponent::GetModule<NigelUI>(const std::string& moduleName);
...
std::shared_ptr<Command> ManagerComponent::CreateCommand(Command* command)
{
	if (command)
	{
		...
		{
			Console.Warning("duplicate command name detected for \"" + commandName + "\"");
		}
	}
	else
	{
		Console.Error("failed to create command, given pointer is invalid");
	}
...
std::shared_ptr<Setting> ManagerComponent::CreateSetting(Setting* setting)
{
	if (setting)
	{
		...
		{
			Console.Warning("duplicate setting name detected for \"" + settingName + "\"");
		}
	}
	else
	{
		Console.Error("failed to create setting - invalid pointer");
	}
...
void ManagerComponent::Initialize() {
    std::shared_ptr<NigelUI> nigelUI;
    CreateModule<NigelUI>(new NigelUI("NigelUI", "The main UI for Nigel SDK", States::STATES_All), nigelUI);
}
