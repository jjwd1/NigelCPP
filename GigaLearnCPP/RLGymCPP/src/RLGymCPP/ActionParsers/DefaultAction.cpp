...
RLGC::DefaultAction::DefaultAction() {
	constexpr float
		...
		// 7 ground steer values for finer control (dribbles, turns)
		R_S[] = { -1, -0.75f, -0.5f, -0.25f, 0, 0.25f, 0.5f, 0.75f, 1 };
	...
	groundHandbrakeMask.resize(actions.size());
...
std::vector<uint8_t> RLGC::DefaultAction::GetActionMask(const Player& player, const GameState& state) {
	...
	if (player.isOnGround) {
		...
		fnApplyMask(groundHandbrakeMask, false);
	} else {
